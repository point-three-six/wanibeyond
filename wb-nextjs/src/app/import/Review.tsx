    ...
    return (
        <>
            <div className={`mb-5`}>
                ...
                <div className='mt-1 mb-4'>{props.deckName}</div>
            </div>
            <div className={`mb-5`}>
                <label htmlFor='deck' className='text-sm font-medium text-gray-700 dark:text-inherit'>File</label>
                <div className='mt-1 mb-4'>{props.file.name}</div>
            </div>
            <div className={`mb-5`}>
                <label htmlFor='deck' className='text-sm font-medium text-gray-700 dark:text-inherit'>Item Type</label>
                <div className='mt-1 mb-4'>{props.itemType}</div>
            </div>
            <div className={`text-center mt-8 ${props.importRequestStatus == 200 ? 'hidden' : ''}`}>
                <button
                    type="button"
                    className="text-white bg-blue-500 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2"
                    onClick={() => props.onSubmit()}
                >
                    <div className={`mr-1 inline ${props.isImporting ? '' : 'hidden'}`}>
                        <span >
