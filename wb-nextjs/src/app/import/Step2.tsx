'use client';

import React, { useState } from 'react';

export default function Step1(props) {

    function getHeaderRowField() {
        if (!props.isHeaderRowConfirmed) {
            return (
                <div className='mt-1'>
                    <div className='mb-2'>
                        Below is the first row of your selected file.
                    </div>
                    <pre className='my-4 text-sm bg-neutral-100 p-2 dark:bg-neutral-700 dark:text-inherit'>
                        <code>
                            {props.fileData.data[0].join(',')}
                        </code>
                    </pre>
                    <div className='mb-4'>
                        The mapping process can be made easier if there is a header row at the start of your file.
                        <br />Please indicate whether the first row of your file is a header.
                    </div>
                    <div className='mb-4'>
                        <label className='relative inline-flex items-center cursor-pointer'>
                            <input type='checkbox' value='' className='sr-only peer' onChange={props.onHeaderRowChanged} checked={props.hasHeaderRow} />
                            <div className='w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[""] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600'></div>
                            <span className='ml-3 text-sm font-medium text-gray-900 dark:text-gray-300'>
                                {
                                    props.hasHeaderRow ? 'Yes, this is a header row.' : 'No, this is not a header row.'
                                }
                            </span>
                        </label>
                    </div>
                    <button
                        type="button"
                        className="text-white bg-neutral-400 hover:bg-neutral-300 font-medium rounded-lg text-xs px-5 py-2.5 mr-2 mb-2"
                        onClick={() => props.onHeaderRowConfirmed()}
                    >
                        <div className='mr-1 inline hidden'>
                            <span >
                                <svg aria-hidden="true" role="status" className="inline mr-3 w-4 h-4 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB" />
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor" />
                                </svg>
                            </span>
                        </div>
                        Confirm
                    </button>
                </div>
            );
        } else {
            if (props.hasHeaderRow) {
                return (
                    <pre className='mt-2'>
                        <code>
                            {props.fileData.data[0].join(',')}
                        </code>
                    </pre>
                );
            } else {
                return (
                    <div>
                        No header row in file.
                    </div>
                )
            }
        }
    }

    function getSpecialColumnField() {
        if (!props.scPopupClicked) {
            return (
                <>
                    <div className='mb-3'>
                        To give you more control over the importing process, WaniPlus has a list of pre-defined columns that it will look for in your file.
                        These special rows give more information to WaniPlus on how your data should be imported.
                    </div>
                    <div className='mb-5'>
                        <a href="#" target="_blank" className='margin-left-auto  text-orange-600'>Learn more about special columns</a>
                    </div>
                    <button
                        type="button"
                        className="text-white bg-neutral-400 hover:bg-neutral-300 font-medium rounded-lg text-xs px-5 py-2.5 mr-2 mb-2"
                        onClick={() => props.onScPopupClicked()}
                    >
                        <div className='mr-1 inline hidden'>
                            <span >
                                <svg aria-hidden="true" role="status" className="inline mr-3 w-4 h-4 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB" />
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor" />
                                </svg>
                            </span>
                        </div>
                        Okay
                    </button>
                </>
            );
        } else if (props.detectedSpecialColumns.length > 0) {
            return (
                <pre className='mt-2'>
                    <code>
                        {props.detectedSpecialColumns.join(',')}
                    </code>
                </pre>
            );

        } else {
            return <div>No special columns detected.</div>
        }
    }

    return (
        <>
            <div className={`flex mb-5`}>
                <div className={`flex-initial circle mr-6 mt-3 ${props.isHeaderRowConfirmed ? 'checked' : ''}`}>
                    <div className='checkmark'></div>
                </div>
                <div className='flex-1 w-full flex-override'>
                    <label htmlFor='privacy' className='text-sm font-medium text-gray-700 dark:text-inherit'>Header</label>
                    {getHeaderRowField()}
                </div>
            </div>
            <div className={`flex mb-5`}>
                <div className={`flex-initial circle mr-6 mt-3 ${props.scPopupClicked ? 'checked' : ''}`}>
                    <div className='checkmark'></div>
                </div>
                <div className='flex-1 w-full flex-override'>
                    <label htmlFor='privacy' className='text-sm font-medium text-gray-700 dark:text-inherit'>Special Columns</label>
                    {getSpecialColumnField()}
                </div>
            </div>
            <div className={`text-center mt-8 ${!props.isHeaderRowConfirmed || !props.scPopupClicked ? 'hidden' : ''}`}>
                <button
                    type="button"
                    className="text-white bg-blue-500 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2"
                    onClick={() => props.onNext({})}
                >
                    <div className='mr-1 inline hidden'>
                        <span >
                            <svg aria-hidden="true" role="status" className="inline mr-3 w-4 h-4 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB" />
                                <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor" />
                            </svg>
                        </span>
                    </div>
                    continue
                </button>
            </div >
        </>
    )
}
